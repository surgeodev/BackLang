...
export function activate(context: vscode.ExtensionContext) {
    console.log('BackLang extension activated');
    ...
    item.tooltip = blPath ? `Binary: ${blPath}` : 'bl not found in PATH — run bl --install or BackLang: Build';
...
function findBlBinary(): string | null {
    ...
    try {
        const out = execSync(`${whichCmd} bl`, { encoding: 'utf-8', timeout: 3000 }).trim();
    ...
    if (process.platform === 'win32') {
        candidates.push(
            path.join(process.env.USERPROFILE || '', 'bin', 'bl.exe'),
            path.join(process.env.USERPROFILE || '', '.cargo', 'bin', 'bl.exe'),
        );
    } else {
        candidates.push('/usr/local/bin/bl', '/usr/bin/bl');
    }
...
async function buildRelease() {
    ...
    t.show();
    t.sendText(`cargo build --release`);
}
...
async function askBuild() {
    const r = await vscode.window.showErrorMessage('BackLang binary not found. Build from source?', 'Build');
...
// ── Templates ────────────────────────────────────
const BACKEND_TEMPLATE = `// BackLang API
